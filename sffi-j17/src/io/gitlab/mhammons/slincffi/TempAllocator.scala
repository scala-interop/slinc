...
final class TempAllocator:
   ...
   private var currentSegment =
      ...
      CLinker.allocateMemory(next).nn.asSegment(next, ResourceScope.globalScope.nn).nn
   ...
   def allocate(bytes: Long, alignment: Long): MemorySegment =
      if bytes + offset > currentSegment.byteSize then
         ...
         currentSegment =
            MemorySegment.allocateNative(nextSize, alignment,
...
//   def get[A](fn: FFI17.Scope ?=> A): A = 
//     val alloc = localAllocator
//     val res = fn(using FFI17.Scope17(ResourceScope.globalScope().nn, alloc))
//     reset()
//     res
