...
class GrowingArray[T <: AnyRef: ClassTag](originalSize: Int) {
   private val array = AtomicReference(Array.fill[AtomicMarkableReference[T]](originalSize)(AtomicMarkableReference[T](null.asInstanceOf[T], false)))
   ...
   def resizeArray(desiredIndex: Int):Unit =
      ...
      val success = if arr.length <= desiredIndex then 
         ...
         while proposedLength <= desiredIndex do
            proposedLength *= 2
         println(arr.length)
         val nArr = Array.fill[AtomicMarkableReference[T]](proposedLength)(AtomicMarkableReference[T](null.asInstanceOf[T], false))
